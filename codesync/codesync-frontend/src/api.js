...
export const api = {
  ...
  createRoom: (name) => req("POST", "/api/rooms", { name: name || "" }),
  getRoom: (roomId) => req("GET", "/api/rooms/" + roomId),
  listRooms: () => req("GET", "/api/rooms"),
  saveCode: (roomId, lang, code) => req("PATCH", "/api/rooms/" + roomId + "/code", { lang, code }),
};